...
//    }
    @Override
    @Transactional // 임시저장
    public FilterCreateDto.TmpFilterResponse saveTemporaryFilter(UUID userId, MultipartFile thumbnailImg, List<MultipartFile> representationImgs, FilterCreateDto.CreateTmpFilterRequest requestDto){

        //속성 설정
        TemporaryFilter temporaryFilter = saveFilterAttribute(userId, requestDto);
    ...
    @Override
    @Transactional
    public FilterCreateDto.TmpFilterResponse saveFilter(UUID userId, MultipartFile thumbnail, List<MultipartFile> representationImg, FilterCreateDto.CreateTmpFilterRequest requestDto){

    }



    public TemporaryFilter saveFilterAttribute(UUID userId, FilterCreateDto.CreateTmpFilterRequest requestDto){
        User user = userRepository.findByUserId(userId).orElseThrow(() -> new UserException(UserErrorResult.NOT_FOUND_USER));
    ...
    public void saveTempFilterInformation(UUID temporaryFilterId, FilterCreateDto.CreateTmpFilterRequest requestDto){
        TemporaryFilter temporaryFilter = temporaryFilterRepository.findByTemporaryFilterId(temporaryFilterId).orElseThrow(() -> new FilterException(FilterErrorResult.NOT_FOUND_TEMPORARY_FILTER));
        ...
        temporaryFilter.updateTemporaryFilterInfo(requestDto.getFilterInformation().getName(), requestDto.getFilterInformation().getDescription());
        temporaryFilterRepository.save(temporaryFilter);
